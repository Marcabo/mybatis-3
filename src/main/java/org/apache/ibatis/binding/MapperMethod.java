  ...
  public static class SqlCommand {
    ...
    public SqlCommand(Configuration configuration, Class<?> mapperInterface, Method method) {
      ...
      if (ms == null) {
        ...
        } else { // 如果找不到 MappedStatement , 则抛出 BindingException
        ...
        if (type == SqlCommandType.UNKNOWN) { // 如果是 UNKONW 类型, 抛出 BindingException
    ...
    private MappedStatement resolveMappedStatement(Class<?> mapperInterface, String methodName,
      ...
      // 如果有,获得 MappedStatement 对象, 并且返回
    ...
    private Integer getUniqueParamIndex(Method method, Class<?> paramType) {
      ...
      for (int i = 0; i < argTypes.length; i++) {
        if (paramType.isAssignableFrom(argTypes[i])) { // 类型符合
          ...
          if (index == null) {
            index = i;
            // 如果类型重复了,抛出 BindingException 异常
          } else {
